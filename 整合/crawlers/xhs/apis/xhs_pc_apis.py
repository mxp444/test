...
class XHS_Apis():
    ...
    def get_user_all_notes(self, user_url: str, cookies_str: str, proxies: dict = None):
        ...
        try:
            ...
            kvDist = {
                key: value[0]
                for key, value in urllib.parse.parse_qs(urlParse.query).items()
            }
    ...
    def get_user_like_note_info(self, user_id: str, cursor: str, cookies_str: str, xsec_token='', xsec_source='', proxies: dict = None):
        ...
        try:
            ...
            msg = res_json.get("msg", res_json.get("code", ""))
        except Exception as e:
            success = False
            msg = str(e)
        return success, msg, res_json
    ...
    def search_note(self, query: str, cookies_str: str, page=1, sort_type_choice=0, note_type=0, note_time=0, note_range=0, pos_distance=0, geo="", proxies: dict = None):
        ...
        try:
            ...
            if "response_text" in locals() and response_text:
                snippet = " ".join(str(response_text).strip().split())[:220]
                detail.append(f"body={snippet}")
            detail.append(f"error={e}")
            trace = traceback.format_exc(limit=4).strip().replace("\n", " | ")
            if trace:
                detail.append(f"trace={trace[:500]}")
            msg = " | ".join(detail)
        return success, msg, res_json
