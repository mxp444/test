...
def make_weibo_shape(thread: Any, forum: str, page: int) -> Dict[str, Any]:
    ...
    thread_key = f"tieba:thread:{tid}" if tid else ""
...
async def run_async_crawler(controller, log, progress, item_callback=None) -> Dict[str, int]:
    ...
    try:
        async with tb.Client() as tieba_client:
            ...
            while forum_states and (max_items <= 0 or inserted < max_items):
                controller.poll()
                ...
                await asyncio.sleep(float(getattr(setting, "DOWNLOAD_DELAY", 0.5)))
            if max_items > 0 and inserted >= max_items:
                log(f"已达到本平台本轮上限 {max_items} 条，停止贴吧爬取。")
