...
class CrawlTask:
    ...
    def _run(self):
        try:
            result = xhs_crawler.run_crawler(controller=self.controller, log=self.add_log, progress=self.update_stats)
            with self.lock:
...
def media_url(path):
    ...
    try:
        media_path = media_path.relative_to(PROJECT_DIR)
    except ValueError:
        pass
...
def resolve_media_path(file_path):
    path = Path(unquote(str(file_path)))
    ...
    if marker in normalized:
        ...
        migrated_path = PROJECT_DIR / "result" / tail
