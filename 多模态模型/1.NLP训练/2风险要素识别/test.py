...
# ================= 配置区域 =================
WORD2VEC_PATH = "sgns.financial.word"
SIMILARITY_THRESHOLD = 0.40  # 略微降低阈值以适应旧词库

# ================= 优化后的16大风险类别及种子词 =================
# 优化原则：1. 去掉缩写/谐音 2. 去掉近年网络新词 3. 回归经典金融术语
category_seeds = {
    # 1. 保本无风险虚假承诺类
    "保本无风险虚假承诺类": [
        "保本", "保息", "零风险", "无风险", "保证", "担保",
...
def build_category_vectors(model, category_seeds):
    """基于种子词计算每个类别的平均向量"""
    category_vectors = {}
    print("\n正在构建类别向量...")
    for category, seeds in category_seeds.items():
        valid_seeds = [seed for seed in seeds if seed in model]
        if valid_seeds:
            vectors = [model[seed] for seed in valid_seeds]
            category_vectors[category] = np.mean(vectors, axis=0)
            print(f"[OK] {category}: {len(valid_seeds)}/{len(seeds)}")
        else:
            print(f"[WARN] {category} 构建失败")
    print(f"\n成功构建 {len(category_vectors)} 个类别\n")
    return category_vectors


def classify_word(word, model, category_vectors, threshold):
    """判断单个词的风险类别"""
...
def analyze_text(text, model, category_vectors, threshold=SIMILARITY_THRESHOLD):
    """分析完整文本"""
    words = jieba.lcut(text)

    # 初始化结果
    result = {cat: [] for cat in category_vectors.keys()}
    result["未分类"] = []
    ...
    for word in words:
        ...
        category, score = classify_word(word, model, category_vectors, threshold)
        if category:
            # 简单去重
...
if __name__ == "__main__":
    model = load_model(WORD2VEC_PATH)

    if model:
        category_vectors = build_category_vectors(model, category_seeds)

        if category_vectors:
            print("=" * 40)
            # 测试文本：去掉了vx，换成了更书面的表达
            demo_text = "最后机会！内部渠道保本保息高收益理财，年化收益可观，轻松赚钱不是梦！赶紧联系我们，推荐朋友发展下线还有额外返利分红，错过不再有！"

            print(f"【检测文本】: {demo_text}\n")
            result, summary = analyze_text(demo_text, model, category_vectors)
